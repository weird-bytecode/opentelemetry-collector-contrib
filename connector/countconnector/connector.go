...
func (c *count) ConsumeTraces(ctx context.Context, td ptrace.Traces) error {
	...
	if c.sizeTraces && c.tracesSizer != nil {
		sizeMetrics := pmetric.NewMetrics()
		sizeResource := sizeMetrics.ResourceMetrics().AppendEmpty()
		sizeScope := sizeResource.ScopeMetrics().AppendEmpty()
		// TODO: Extract to own module, and/or entirely different connector
		// TODO: I think this may be naïve and dependent on the batchprocessor, because without `ld` being batched, wildly different sums could flow through here
		sizeMetric := sizeScope.Metrics().AppendEmpty()
		sizeMetric.SetName("metrics_size_bytes")
		sizeMetric.SetDescription("Size of metrics in bytes")
		...
		/**
		 TODO:
				Likely an extreme case, but what if the cumulative size is huge?
				Like, greater than int64 huge?
				Is that even practical to calculate with plog.logsSizer anyways?

				How could we introduce a circuit-breaker to keep from hanging on log batches
				too large to size?
		*/
		sizeDatapoint.SetIntValue(int64(c.tracesSizer.TracesSize(td)))
		sizeDatapoint.SetTimestamp(pcommon.NewTimestampFromTime(time.Now()))
		if multiError != nil {
			return multiError
		}
		return c.metricsConsumer.ConsumeMetrics(ctx, sizeMetrics)
	} else {
...
func (c *count) ConsumeMetrics(ctx context.Context, md pmetric.Metrics) error {
	...
	if c.sizeMetrics && c.metricsSizer != nil {
		sizeMetrics := pmetric.NewMetrics()
		sizeResource := sizeMetrics.ResourceMetrics().AppendEmpty()
		sizeScope := sizeResource.ScopeMetrics().AppendEmpty()
		// TODO: Extract to own module, and/or entirely different connector
		...
		/**
		 TODO:
				...
				too large to size?
		*/
		sizeDatapoint.SetIntValue(int64(c.metricsSizer.MetricsSize(md)))
		sizeDatapoint.SetTimestamp(pcommon.NewTimestampFromTime(time.Now()))
...
func (c *count) ConsumeLogs(ctx context.Context, ld plog.Logs) error {
	...
	if c.sizeLogs && c.logsSizer != nil {
		...
		// TODO: I think this may be naïve and dependent on the batchprocessor, because without `ld` being batched, wildly different sums could flow through here
		sizeMetric := sizeScope.Metrics().AppendEmpty()
		sizeMetric.SetName("logs_size_bytes")
